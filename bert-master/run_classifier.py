...
flags.DEFINE_string(
    "bert_config_file", './inputs/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'ad', "The name of the task to train.")

flags.DEFINE_string("vocab_file", './inputs/vocab.txt',
...
flags.DEFINE_string(
    "init_checkpoint", './inputs/bert_model.ckpt',
...
flags.DEFINE_bool("do_train_and_eval", None, "Whether to run training and eval.")
...
class AdProcessor(DataProcessor):
    """Processor for the ad review sentiment analysis task"""
    # read txt
    ...
    def get_train_examples(self, data_dir):
        ...
        for line in f.readlines():
            ...
            line = line.replace("\n", "").split("\t")
            text_a = tokenization.convert_to_unicode(str(line[0]))#要分类的文本
            label = str(line[1])#文本对应的类别
...
def main(_):
    ...
    if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_train_and_eval:
        raise ValueError(
            "At least one of `do_train`, `do_eval` or `do_predict' or 'do_train_and_eval' must be True.")
    ...
    if FLAGS.do_train_and_eval:
        ...
        tensors_to_log = {"train loss": "train_loss","eval_loss":"eval_loss"}
        ...
        tf.estimator.train_and_evaluate(
            estimator,
            train_spec=tf.estimator.TrainSpec(train_input_fn, hooks=[early_stopping, logging_hook]),
